...
from rag_latest.pipeline import run_all


class PipelineTest(unittest.TestCase):
    @mock.patch("rag_latest.pipeline.index_articles")
    @mock.patch("rag_latest.pipeline.db.load_article_ids_by_urls", side_effect=[[], [11, 12]])
    @mock.patch("rag_latest.pipeline.insert_articles", return_value=2)
    @mock.patch("rag_latest.pipeline.fetch_news")
    def test_이번에_수집한_URL의_기사를_GLiNER2_indexer로_넘긴다(
        self,
        fetch_news,
        insert_articles,
        load_article_ids_by_urls,
        index_articles,
    ):
        ...
        result = run_all("AI", 2, 5, device="cpu")
        ...
        self.assertEqual(
            ...
            load_article_ids_by_urls.call_args_list,
        )
    ...
    @mock.patch("rag_latest.pipeline.index_articles", return_value=[])
    @mock.patch("rag_latest.pipeline.db.load_article_ids_by_urls", side_effect=[[7], [7]])
    @mock.patch("rag_latest.pipeline.insert_articles", return_value=0)
    @mock.patch(
        "rag_latest.pipeline.fetch_news",
    ...
    def test_이미_저장된_URL은_다시_indexing하지_않는다(
        ...
        _fetch_news,
        _insert_articles,
        _load_article_ids_by_urls,
        index_articles,
    ):
        result = run_all("AI", device="cpu")
