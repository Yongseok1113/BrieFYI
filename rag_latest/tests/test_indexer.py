...
from rag_latest.content import ArticleContentDependencyError
from rag_latest.indexer import _source_fingerprint, index_all_articles, index_articles, index_events
from rag_latest.taxonomy import (
    EVENT_EXTRACTION_VERSION,
    EVENT_TAXONOMY_VERSION,
    GLINER2_MODEL_NAME,
)
from rag_latest.tests.test_content import OffsetTokenizer
...
class IndexArticlesTest(unittest.TestCase):
    ...
    def test_본문_수집에_성공하면_본문을_청킹한다(self):
        with (
            mock.patch("rag_latest.indexer.db.load_articles", return_value=[self.ARTICLE]),
            mock.patch("rag_latest.indexer.load_embedding_tokenizer", return_value=OffsetTokenizer()),
            mock.patch("rag_latest.indexer.load_gliner2_model"),
            mock.patch("rag_latest.indexer.GLiNER2TopicExtractor") as topic_extractor,
            mock.patch("rag_latest.indexer.GLiNER2EventExtractor"),
            mock.patch("rag_latest.indexer.fetch_article_body", return_value="실제 기사 본문"),
            mock.patch("rag_latest.indexer.embed_texts", return_value=[[1.0, 0.0, 0.0]]) as embed,
            mock.patch("rag_latest.indexer.db.store_article_index") as store,
            mock.patch(
                "rag_latest.indexer.index_events",
    ...
    def test_본문_수집에_실패하면_title_description을_사용한다(self):
        with (
            ...
            mock.patch("rag_latest.indexer.GLiNER2TopicExtractor") as topic_extractor,
            mock.patch("rag_latest.indexer.GLiNER2EventExtractor"),
            mock.patch("rag_latest.indexer.fetch_article_body", side_effect=RuntimeError("blocked")),
            mock.patch("rag_latest.indexer.embed_texts", return_value=[[1.0, 0.0, 0.0]]) as embed,
            mock.patch("rag_latest.indexer.db.store_article_index"),
            mock.patch(
                "rag_latest.indexer.index_events",
    ...
    def test_본문_추출_의존성_누락은_fallback으로_숨기지_않는다(self):
        with (
            ...
            mock.patch("rag_latest.indexer.load_embedding_tokenizer", return_value=OffsetTokenizer()),
            mock.patch("rag_latest.indexer.load_gliner2_model"),
            mock.patch("rag_latest.indexer.GLiNER2TopicExtractor"),
            mock.patch(
                "rag_latest.indexer.fetch_article_body",
...
class IndexAllArticlesTest(unittest.TestCase):
    @mock.patch("rag_latest.indexer.index_articles")
    @mock.patch("rag_latest.indexer.db.load_unindexed_article_ids", return_value=[3, 7])
...
class IndexEventsTest(unittest.TestCase):
    @mock.patch("rag_latest.indexer.db.replace_article_events")
    @mock.patch("rag_latest.indexer.load_event_extractor")
    @mock.patch("rag_latest.indexer.db.load_event_statuses", return_value={})
    @mock.patch("rag_latest.indexer.db.load_articles_with_chunks")
    ...
    @mock.patch("rag_latest.indexer.load_event_extractor")
    @mock.patch("rag_latest.indexer.db.load_event_statuses")
    @mock.patch("rag_latest.indexer.db.load_articles_with_chunks")
    ...
    @mock.patch("rag_latest.indexer.db.mark_event_failed")
    @mock.patch("rag_latest.indexer.db.replace_article_events")
    @mock.patch("rag_latest.indexer.load_event_extractor")
    @mock.patch("rag_latest.indexer.db.load_event_statuses", return_value={})
    @mock.patch("rag_latest.indexer.db.load_articles_with_chunks")
